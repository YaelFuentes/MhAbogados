...
const HonorariosCliente = ({ id }) => {
    ...
    if (!honorarios) {
        return <p>Error al cargar los honorarios</p>
    }
    ...
    const handleEditClick = (honorario) => {
        ...
        setOpenModal(true);
        // Set the idExp in the component's state
        setFormData((prevData) => ({ ...prevData, idExp: honorario.idexp }));
    };
    ...
    const handleSaveEdit = async () => {
        try {
            const updatedHonorario = {
                idexp: editingHonorario.idexp, // Assuming idexp is the correct property for idExp
                honoinicial: formData.honoinicial,
                honofinal: formData.honofinal,
                honocamara: formData.honocamara,
            };

            // Use formData.idExp here
            const response = await axios.put(`/api/expedienteClient/expCliente?idexp=${formData.idExp}`, updatedHonorario);
    ...
    console.log(honorarios.expCliente)
